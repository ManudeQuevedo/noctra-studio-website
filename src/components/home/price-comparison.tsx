    ...
    <div className={cn("border-t border-neutral-800/50 pt-4 mt-4", className)}>
      ...
      >
        <div className="flex items-center gap-2">
          {Icon && <Icon className="w-3.5 h-3.5 text-neutral-500 group-hover:text-emerald-500 transition-colors" />}
...
export function PriceComparison() {
  ...
  const activeWarning = useMemo(() => {
    ...
    if (selectedIndustry === 'medical' && selectedOptions.has('doctoralia')) return warnings.doctoralia;
    if (selectedIndustry === 'hospitality' && (selectedOptions.has('hosp_booking') || selectedOptions.has('hosp_airbnb'))) return warnings.hospitality_ota;
  ...
  return (
    <div className="mb-24 px-4 scroll-mt-24">
      ...
      <div className="text-center mb-16">
        ...
        <h3 className="text-4xl md:text-5xl lg:text-6xl font-black mb-6 tracking-tight text-white">{t("title")}</h3>
        <p className="text-neutral-400 max-w-3xl mx-auto text-xl leading-relaxed">{t("subtitle")}</p>
      </div>
            ...
            {industries.map((industry) => {
              ...
              return (
                ...
                >
                  ...
                  <h4 className="text-xl font-bold text-white mb-2 group-hover:text-emerald-400 transition-colors">{industry.label}</h4>
                  <p className="text-sm text-neutral-500 leading-relaxed">{industry.sub}</p>
                  ...
                  <div className="mt-8 flex items-center gap-2 text-xs font-bold text-neutral-600 group-hover:text-emerald-500 transition-all">
                    {industryLabels.select_button} <ChevronRight className="w-4 h-4" />
                  </div>
                </motion.button>
              );
            })}
        ...
        >
          <div className="flex items-center gap-4">
            <div className="w-12 h-12 rounded-xl bg-emerald-500/20 flex items-center justify-center text-emerald-400 border border-emerald-500/30">
              {ICON_MAP[industries.find(i => i.id === selectedIndustry)?.icon || 'LayoutGrid'] && React.createElement(ICON_MAP[industries.find(i => i.id === selectedIndustry)?.icon || 'LayoutGrid'] as any, { className: "w-6 h-6" })}
            </div>
            <div>
              <p className="text-[10px] font-black text-neutral-500 uppercase tracking-widest leading-none mb-1">{industryLabels.selected_badge}</p>
              <h4 className="text-xl font-bold text-white">{industries.find(i => i.id === selectedIndustry)?.label}</h4>
            </div>
          </div>
          <div className="flex items-center gap-4">
            <button 
      ...
      {selectedIndustry && (
        <div id="comparison-dashboard" className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-12 gap-8 mb-16 items-start scroll-mt-24">
          
          {/* Left: Category Selector (7 cols) */}
          <div className="lg:col-span-7 space-y-6">
            {activeCategories.map((cat) => (
              <div key={cat.id} className="p-8 rounded-[2.5rem] bg-neutral-900/30 border border-neutral-800/50 backdrop-blur-sm relative overflow-hidden group">
                ...
                <h4 className="text-xl font-bold text-white mb-8 flex items-center gap-3">
                  <span className="w-8 h-8 rounded-lg bg-neutral-800 flex items-center justify-center text-sm border border-neutral-700">{cat.label.split(' ')[0]}</span>
                    ...
                    return (
                      ...
                      >
                        ...
                        <span className={cn(
                          ...
                          isSelected ? "text-emerald-400" : "text-neutral-200 group-hover:text-white"
                        )}>
                          {opt.label}
                        </span>
                        ...
                        <div className="mt-auto pt-2 flex items-center gap-2">
                           <span className={cn(
                             ...
                             isSelected ? "text-emerald-500/70" : "text-neutral-600"
                           )}>
                ...
                <div className="space-y-6">
                  <div>
                    <div className="text-3xl md:text-4xl font-black text-white mb-1">
                      ...
                      <span className="text-sm font-medium text-neutral-500 ml-2">MXN</span>
                    </div>
                  ...
                  <div className="flex items-center gap-4 py-2">
                    ...
                    <div className="px-3 py-1 rounded-full border border-neutral-700 text-[10px] font-black text-neutral-500 uppercase tracking-widest">VS</div>
                    <div className="h-px bg-neutral-800 flex-1" />
                  </div>
                  ...
                  <div className="p-6 rounded-3xl bg-emerald-500 shadow-[0_20px_40px_-15px_rgba(16,185,129,0.4)] relative overflow-hidden group/noctra">
                    ...
                    <div className="relative z-10">
                      ...
                      <div className="text-3xl font-black text-white mb-1">
                        $20,000
                        <span className="text-sm font-medium text-emerald-100 ml-2">MXN</span>
                      </div>
                      ...
                      <ul className="grid grid-cols-2 gap-y-2 gap-x-4 mb-4">
                        {columns.noctra.included_items.slice(0, 6).map((f: string, i: number) => (
                          <li key={i} className="flex items-center gap-1.5 text-[9px] font-bold text-white/90">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-1 gap-4">
              <div className="p-5 rounded-2xl bg-red-500/5 border border-red-500/20">
                <h5 className="text-[10px] font-black text-red-500 uppercase tracking-widest mb-3 flex items-center gap-2">
                  <AlertTriangle className="w-3 h-3" /> {columns.current.hidden_costs_title}
                </h5>
                <ul className="space-y-2">
                  {columns.current.hidden_costs_items.map((item: string, i: number) => (
              ...
              <div className="p-5 rounded-2xl bg-emerald-500/5 border border-emerald-500/20">
                <h5 className="text-[10px] font-black text-emerald-500 uppercase tracking-widest mb-3 flex items-center gap-2">
                  <ShieldCheck className="w-3 h-3" /> {columns.noctra.zero_hidden_title}
                </h5>
                <ul className="space-y-2">
                  {columns.noctra.zero_hidden_items.map((item: string, i: number) => (
                    <li key={i} className="text-[10px] text-neutral-500 flex items-start gap-2">
        ...
        >
          <div className="flex items-center gap-3">
            ...
            <span className="font-bold text-neutral-200 group-hover:text-white transition-colors">
               {showBreakdown ? breakdownTranslations.cta_close : breakdownTranslations.cta_open}
            </span>
          </div>
          {showBreakdown ? <ChevronUp className="w-5 h-5 text-neutral-500" /> : <ChevronDown className="w-5 h-5 text-neutral-500" />}
        </button>
          ...
          {showBreakdown && (
            ...
            >
              <div className="p-8 rounded-3xl bg-neutral-900/30 border border-neutral-800/50">
                <h4 className="text-xs font-black text-white mb-8 uppercase tracking-[0.2em] text-center">{breakdownTranslations.title}</h4>
                
                <div className="overflow-x-auto">
                  <table className="w-full text-left">
                    <thead>
                      <tr className="border-b border-neutral-800">
                        <th className="py-4 text-[10px] font-black text-neutral-500 uppercase tracking-widest">Item</th>
                        <th className="py-4 px-4 text-[10px] font-black text-neutral-500 uppercase tracking-widest">{breakdownTranslations.year_1}</th>
                        <th className="py-4 px-4 text-[10px] font-black text-neutral-500 uppercase tracking-widest">{breakdownTranslations.year_2}</th>
                        <th className="py-4 px-4 text-right text-[10px] font-black text-neutral-500 uppercase tracking-widest">Total</th>
                      </tr>
                    </thead>
                    <tbody>
                      {costs.items.map((item) => (
                        <tr key={item.id} className="border-b border-neutral-800/30">
                          <td className="py-4 pr-4">
                            <span className="text-[11px] font-bold text-neutral-300 block">{item.label}</span>
                            <span className="text-[9px] text-neutral-500 leading-tight block mt-1">{item.info}</span>
                          </td>
                          <td className="py-4 px-4 text-[11px] text-neutral-400">
                            ${(item.min * 0.6).toLocaleString()} - ${(item.max * 0.6).toLocaleString()}
                          </td>
                          <td className="py-4 px-4 text-[11px] text-neutral-400">
                            ${(item.min * 0.4).toLocaleString()} - ${(item.max * 0.4).toLocaleString()}
                          </td>
                      ...
                      <tr className="bg-emerald-500/5">
                        <td className="py-5 pr-4 pl-4 border-l-2 border-emerald-500">
                          <span className="text-[11px] font-black text-emerald-400 uppercase tracking-widest">Noctra Studio</span>
                        </td>
                        <td className="py-5 px-4 text-[11px] font-black text-white">$20,000</td>
                        <td className="py-5 px-4 text-[11px] font-black text-white">$0</td>
                        <td className="py-5 px-4 text-right text-[11px] font-black text-emerald-400">$20,000</td>
                      </tr>
                    </tbody>
                    <tfoot>
                ...
                <div className="mt-8 pt-8 border-t border-neutral-800/50 grid grid-cols-1 md:grid-cols-2 gap-8 text-[10px]">
                  <div>
                    <h5 className="font-black text-amber-500 uppercase tracking-widest mb-4 flex items-center gap-2">
                       <AlertTriangle className="w-3 h-3" /> {breakdownTranslations.extra_considerations}
                    </h5>
                    <ul className="space-y-2">
                      {breakdownTranslations.extra_items.map((h: string, i: number) => (
                        <li key={i} className="text-neutral-500 flex items-start gap-2">
                  ...
                  <div>
                    ...
                    <ul className="space-y-2">
                      <li className="text-neutral-500 flex items-start gap-2">
                        <span className="text-emerald-500/50">•</span> Todo integrado en una sola plataforma
                      </li>
                      <li className="text-neutral-500 flex items-start gap-2">
                        <span className="text-emerald-500/50">•</span> Un solo punto de contacto para todo
                      </li>
                      <li className="text-neutral-500 flex items-start gap-2">
                        <span className="text-emerald-500/50">•</span> Optimización continua de performance
                      </li>
                    </ul>
                  </div>
                </div>
         ...
         <div className="max-w-5xl mx-auto mb-20 px-4">
           ...
           >
             ...
             <p className="text-neutral-400 mb-8 max-w-xl mx-auto relative z-10 text-lg leading-relaxed italic">
               "{savingsCard.enough_to_items[Math.floor(Math.random() * savingsCard.enough_to_items.length)]}"
             </p>
             ...
             >
               Calcular ROI exacto para mi negocio <ArrowRight className="w-4 h-4 group-hover:translate-x-1 transition-transform" />
             </button>
           </motion.div>
         </div>
      )}
              ...
              <div className="pt-8 grid grid-cols-1 md:grid-cols-2 gap-12">
                <div className="space-y-8">
                  <div>
                    ...
                    <div className="space-y-6">
                      {methodologyData.sections.map((section: any, idx: number) => (
                        <div key={idx} className="space-y-3">
                          <h5 className="text-[10px] font-black text-white uppercase tracking-widest">{section.title}</h5>
                          <ul className="space-y-2">
                            {section.items?.map((item: string, i: number) => (
                              <li key={i} className="text-[11px] text-neutral-500 flex items-start gap-2 leading-relaxed">
                ...
                <div className="space-y-8">
                  <div className="p-8 rounded-3xl bg-neutral-900/50 border border-neutral-800/50 space-y-6">
                    <div>
                      <h5 className="text-[10px] font-black text-white uppercase tracking-widest mb-3">CONVERSIÓN Y MONEDA</h5>
                      <p className="text-[11px] text-neutral-500 leading-relaxed">{methodologyData.currency}</p>
                    </div>
                    <div>
                      <h5 className="text-[10px] font-black text-white uppercase tracking-widest mb-3">VARIACIONES</h5>
                      <p className="text-[11px] text-neutral-500 leading-relaxed">{methodologyData.variations}</p>
                    </div>
                    <div>
                      <h5 className="text-[10px] font-black text-white uppercase tracking-widest mb-3">COMPROMISO</h5>
                      <p className="text-[11px] text-neutral-500 leading-relaxed">{methodologyData.commitment}</p>
                    </div>
                    <div className="pt-4 border-t border-neutral-800 flex justify-between items-center">
                      <span className="text-[9px] text-neutral-600 font-bold italic">{methodologyData.last_updated}</span>
                      <a href="mailto:hello@noctra.studio" className="text-[9px] font-black text-emerald-500 uppercase tracking-widest hover:text-white transition-colors flex items-center gap-1">
                        Reportar Inexactitud <ExternalLink className="w-2.5 h-2.5" />
                      </a>
                    </div>
                  </div>
                </div>
