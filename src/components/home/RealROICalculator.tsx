...
export function RealROICalculator() {
  ...
  const [revenueTemp, setRevenueTemp] = useState(DEFAULT_STATE.revenue.toString());
  const [isTicketFocused, setIsTicketFocused] = useState(false);
  const [ticketTemp, setTicketTemp] = useState(DEFAULT_STATE.avgTicket.toString());
  ...
  const convertVal = (amount: number, from: 'MXN' | 'USD', to: 'MXN' | 'USD') => {
  ...
  const metrics = useMemo(() => {
    ...
    const ownerHourlyRate = currency === 'MXN' ? 500 : Math.round(500 / CONFIG.EXCHANGE_RATE);
    const websiteInvestment = convertVal(CONFIG.noctraPricing.professional, 'MXN', currency);
  ...
  return (
    <section id="roi-calculator" className="w-full py-24 px-6 md:px-8 border-t border-neutral-900 bg-black/50 overflow-hidden print:bg-white print:text-black">
          ...
          <div className="flex flex-col md:flex-row items-center justify-between gap-6 mb-16">
            ...
            <div className="flex flex-col items-end gap-2 shrink-0">
              <div className="flex p-1 bg-neutral-900 border border-neutral-800 rounded-xl">
                <button
                  ...
                  className={cn(
                    ...
                    currency === 'MXN' ? "bg-emerald-500 text-black" : "text-neutral-500 hover:text-white"
                  )}
                >
                  MXN
                </button>
                <button
                  ...
                  className={cn(
                    "px-4 py-2 rounded-lg text-xs font-mono font-bold transition-all",
                    currency === 'USD' ? "bg-emerald-500 text-black" : "text-neutral-500 hover:text-white"
                  )}
                >
                  USD
                </button>
              ...
              <div className="text-[10px] font-mono text-neutral-500 uppercase tracking-widest text-right">
                1 USD = {formatCur(CONFIG.EXCHANGE_RATE, 'MXN')} | {CONFIG.LAST_UPDATED}
              </div>
            </div>
          </div>
            ...
            {step === 2 && (
              ...
              >
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-12">
                  <div className="space-y-8">
                    ...
                    <div className="space-y-4">
                      <div className="flex justify-between items-end">
                        <div className="space-y-1">
                          <label className="text-xs font-mono uppercase tracking-[0.2em] text-neutral-400">Monthly Revenue</label>
                          <p className="text-[10px] text-neutral-500">Average monthly income from your business</p>
                        </div>
                        <span className="text-[10px] font-mono text-emerald-500/50 bg-emerald-500/5 px-2 py-0.5 rounded border border-emerald-500/10">Required</span>
                      </div>
                      <div className="relative group">
                        ...
                        <Input 
                          type="text" 
                          value={isRevenueFocused ? revenueTemp : formatCur(state.revenue)} 
                          onFocus={() => {
                          ...
                          onBlur={() => {
                            ...
                            const val = parseInt(revenueTemp.replace(/[^0-9]/g, '')) || 1000;
                            const clamped = Math.min(Math.max(val, 1000), currency === 'MXN' ? 10000000 : 556000);
                            setState(s => ({
                              ...s, 
                              revenue: clamped,
                    ...
                    <div className="space-y-4">
                      <div className="flex justify-between items-end">
                        <div className="space-y-1">
                          <label className="text-xs font-mono uppercase tracking-[0.2em] text-neutral-400">Average Transaction</label>
                          <p className="text-[10px] text-neutral-500">Typical amount per sale or project</p>
                        </div>
                      </div>
                      <div className="relative group">
                        <div className="absolute left-4 top-1/2 -translate-y-1/2 w-8 h-8 rounded-lg bg-neutral-800 flex items-center justify-center border border-neutral-700 group-focus-within:border-emerald-500/50 transition-colors">
                        ...
                        <Input 
                          type="text" 
                          value={isTicketFocused ? ticketTemp : formatCur(state.avgTicket)} 
                          onFocus={() => {
                          ...
                          onBlur={() => {
                            ...
                            const val = parseInt(ticketTemp.replace(/[^0-9]/g, '')) || 100;
                      ...
                      <p className="text-[9px] text-neutral-600 font-mono italic">
                        e.g., {formatCur(currency === 'MXN' ? 2500 : 139)} for services | {formatCur(currency === 'MXN' ? 450 : 25)} retail
                      </p>
                    </div>
                    ...
                    <div className="p-6 rounded-2xl bg-neutral-900/50 border border-neutral-800/50 flex items-center justify-between opacity-80">
                      <div className="space-y-1">
                        <div className="flex items-center gap-2">
                          ...
                          <label className="text-[10px] uppercase font-bold text-neutral-500 tracking-widest">Monthly Clients</label>
                        </div>
                        <p className="text-[9px] text-neutral-600 font-mono">
                          {formatCur(state.revenue)} ÷ {formatCur(state.avgTicket)}
                        </p>
                      </div>
                  ...
                  <div className="space-y-8">
                    ...
                    <div className="space-y-6">
                      <div className="flex justify-between items-center bg-neutral-900/50 p-4 rounded-2xl border border-neutral-800">
                        <div className="space-y-1">
                          <div className="flex items-center gap-2">
                            <label className="text-xs font-mono uppercase tracking-[0.2em] text-neutral-400">Conversion Rate</label>
                            <button className="group relative">
                              ...
                              <div className="absolute left-1/2 -translate-x-1/2 bottom-full mb-4 w-[280px] p-4 bg-neutral-900 border border-neutral-800 rounded-2xl shadow-2xl opacity-0 scale-95 pointer-events-none group-hover:opacity-100 group-hover:scale-100 transition-all z-50 text-left">
                                ...
                                <p className="text-[10px] text-neutral-400 leading-relaxed mb-3">
                                  The % of visitors who take action (form, purchase, call).
                                </p>
                                ...
                                <div className="mt-3 p-2 bg-emerald-500/5 rounded-lg border border-emerald-500/10">
                                  <p className="text-[9px] text-emerald-500 italic">
                                    💡 A professional site typically improves this by +40%.
                                  </p>
                                </div>
                              </div>
                            </button>
                          ...
                          <div className={cn(
                            ...
                            state.conversion <= 1 ? "text-red-500" :
                            state.conversion <= 2 ? "text-yellow-500" :
                            "text-emerald-500"
                          )}>
                            {state.conversion <= 1 ? "Poor / Unknown" :
                             state.conversion <= 2 ? "Average / Decent" :
                             "Optimized / High Performance"}
                          </div>
                        </div>
                        <div className="text-3xl font-black text-white pr-2">{state.conversion}%</div>
                      </div>

                      <div className="grid grid-cols-1 gap-2">
                        {[
                          { val: 0.5, label: "No website yet / Minimal presence", sub: "Very conservative starting point" },
                          { val: 1.0, label: "Basic / Old website", sub: "Outdated design, slow on mobile" },
                          { val: 1.5, label: "Decent / Standard website", sub: "Responsive and clean, but unoptimized" },
                          { val: 2.5, label: "Modern / Performance focused", sub: "Strong branding, fast load times" },
                          { val: 4.0, label: "Optimized / High conversion", sub: "A/B tested, conversion lead machine" }
                        ].map((preset) => (
                          <button
                            key={preset.val}
                            onClick={() => setState(s => ({...s, conversion: preset.val}))}
                            className={cn(
                          ...
                          >
                            <div className="space-y-0.5">
                              <div className={cn(
                                ...
                                state.conversion === preset.val ? "text-white" : "text-neutral-400 group-hover:text-neutral-300"
                              )}>
                                {preset.label}
                              </div>
                              <p className="text-[9px] text-neutral-600 line-clamp-1">{preset.sub}</p>
                            </div>
                            <div className={cn(
                              "w-4 h-4 rounded-full border flex items-center justify-center shrink-0 ml-4",
                              state.conversion === preset.val ? "border-emerald-500 bg-emerald-500" : "border-neutral-700"
                            )}>
                              {state.conversion === preset.val && <CheckCircle2 className="w-2.5 h-2.5 text-black" />}
                            </div>
                          </button>
                        ))}
                      </div>
                ...
                <div className="mt-12 flex flex-col sm:flex-row justify-end gap-4">
                  <Button variant="outline" onClick={prevStep} className="h-14 px-8 rounded-2xl border-neutral-800 hover:bg-neutral-800 uppercase tracking-widest font-mono text-[10px]">
                    <ArrowLeft className="w-4 h-4 mr-2" /> Back
                  </Button>
                  <Button onClick={nextStep} className="h-14 px-12 rounded-2xl bg-white text-black hover:bg-neutral-200 uppercase tracking-widest font-bold text-xs shadow-[0_20px_40px_-10px_rgba(255,255,255,0.2)]">
            ...
            {step === 3 && (
              ...
              >
                ...
                <div className="mt-12 flex flex-col sm:flex-row justify-end gap-4">
                  ...
                  <Button onClick={nextStep} className="h-14 px-12 rounded-2xl bg-emerald-500 text-black hover:bg-emerald-400 uppercase tracking-widest font-bold text-xs shadow-[0_20px_40px_-10px_rgba(16,185,129,0.3)]">
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="relative group">
        ...
        <Input 
          type="text" 
          value={isFocused ? tempValue : new Intl.NumberFormat('en-US').format(value)} 
