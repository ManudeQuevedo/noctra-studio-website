...
import { Check, Minus, ChevronDown, HelpCircle } from "lucide-react";
...
export function ComparisonTable() {
  ...
  const features = t.raw("features") as Array<{ name: string; values: (string | boolean)[] }>;
    ...
    <section className="py-24 px-6 bg-neutral-950/50">
      <div className="max-w-6xl mx-auto">
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="border-b border-neutral-800 bg-neutral-900/50">
                ...
                {labels.map((label, i) => (
                  <th key={i} className="p-8 text-center text-sm font-black text-white uppercase tracking-widest">
                    {label}
                  </th>
                ))}
              </tr>
                  ...
                  {feature.values.map((val, j) => (
                    <td key={j} className="p-8 text-center">
                      {typeof val === "boolean" ? (
                        ...
                        <span className={cn(
                          "text-sm font-medium",
                          j === 0 ? "text-neutral-400" : 
                          j === 1 ? "text-neutral-300" : "text-emerald-400"
                        )}>
                          {val}
                        </span>
                      )}
        ...
        <div className="md:hidden space-y-4">
          {features.map((feature, i) => (
            <div key={i} className="rounded-2xl border border-neutral-800 bg-neutral-900/20 overflow-hidden">
              <button 
        ...
        <div className="mt-12 text-center">
          ...
          <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
            <Link href="/contact" className="w-full sm:w-auto px-8 py-3 rounded-xl bg-white text-black font-bold text-sm hover:bg-emerald-50 transition-colors">
              {t("cta_consult")}
            </Link>
            <button 
              onClick={openQuiz}
