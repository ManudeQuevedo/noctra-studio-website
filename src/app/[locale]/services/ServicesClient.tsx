...
const StandardIcons = {
  Performance: () => (
    <div className="relative w-16 h-16 mx-auto mb-6 group">
      <svg viewBox="0 0 64 64" fill="none" xmlns="http://www.w3.org/2000/svg" className="w-full h-full">
        <circle cx="32" cy="32" r="28" stroke="currentColor" strokeWidth="2" className="text-neutral-800" />
        <path d="M32 8V12" stroke="currentColor" strokeWidth="2" className="text-neutral-700" />
        <path d="M56 32H52" stroke="currentColor" strokeWidth="2" className="text-neutral-700" />
        <path d="M32 56V52" stroke="currentColor" strokeWidth="2" className="text-neutral-700" />
        <path d="M8 32H12" stroke="currentColor" strokeWidth="2" className="text-neutral-700" />
        <motion.path 
          d="M32 32L50 14" 
          stroke="#10b981" 
          strokeWidth="3" 
        ...
        <text x="32" y="48" textAnchor="middle" className="text-[8px] font-black fill-emerald-500/50 uppercase tracking-widest">1.5s</text>
      </svg>
  ...
  Worldwide: () => (
    <div className="relative w-16 h-16 mx-auto mb-6 group">
      <svg viewBox="0 0 64 64" fill="none" xmlns="http://www.w3.org/2000/svg" className="w-full h-full">
        <circle cx="32" cy="32" r="28" stroke="currentColor" strokeWidth="1" className="text-neutral-800" />
        <ellipse cx="32" cy="32" rx="12" ry="28" stroke="currentColor" strokeWidth="1" className="text-neutral-800" />
        <line x1="4" y1="32" x2="60" y2="32" stroke="currentColor" strokeWidth="1" className="text-neutral-800" />
        <motion.circle 
          cx="32" cy="12" r="3" 
          fill="#10b981"
        ...
        />
        <path d="M32 12L52 40L12 40Z" stroke="#10b981" strokeWidth="1" strokeDasharray="4 4" className="opacity-30" />
      </svg>
    </div>
  ),
  Secure: () => (
    <div className="relative w-16 h-16 mx-auto mb-6 group">
      <svg viewBox="0 0 64 64" fill="none" xmlns="http://www.w3.org/2000/svg" className="w-full h-full">
        <path d="M32 6L10 14V30C10 44 32 58 32 58C32 58 54 44 54 30V14L32 6Z" stroke="#10b981" strokeWidth="2" className="fill-emerald-500/5" />
        <motion.path 
        ...
        />
        <rect x="42" y="10" width="16" height="10" rx="4" className="fill-emerald-500" />
        <text x="50" y="17" textAnchor="middle" className="text-[6px] font-black fill-black lowercase">99.9%</text>
      </svg>
    </div>
  )
};
            ...
            <div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                {data.deliverables.map((item: string, i: number) => (
                  <div key={i} className="px-4 py-3 rounded-xl bg-white/[0.03] border border-white/[0.05] text-sm text-white font-medium flex items-center gap-2">
              ...
              <div className="space-y-4">
                {data.timeline.map((step: string, i: number) => (
                  <div key={i} className="flex items-center gap-4 group/step">
                    ...
                    <span className="text-sm text-neutral-400 group-last:text-white group-last:font-bold">{step}</span>
                  </div>
                ))}
              </div>
            </div>
            ...
            <div className="pt-8 border-t border-neutral-800">
              ...
              {data.investment ? (
                <div className="space-y-3">
                  {data.investment.filter((inv: any) => !inv.total).map((inv: any, i: number) => (
                    <div key={i} className="flex justify-between text-sm py-1 border-b border-white/[0.03]">
                      <span className="text-neutral-500">{inv.label}</span>
                      <span className="text-white font-mono">{inv.value}</span>
                    </div>
                  ))}
                  <div className="flex justify-between items-center pt-8 text-2xl font-black text-white">
                    <span>{labels.total}</span>
                    <span className="text-emerald-500">{data.investment.find((inv: any) => inv.total)?.total}</span>
                  </div>
                </div>
              ) : (
                <div className="space-y-4">
                  {data.investment_notes.map((note: string, i: number) => (
                    <p key={i} className="text-sm text-neutral-400 italic font-medium">
                      {note}
                    </p>
                  ))}
                </div>
        ...
        <div className="md:col-span-5 relative group">
          <div className="relative w-full aspect-[4/5] rounded-[2.5rem] overflow-hidden border border-neutral-800 shadow-2xl">
            ...
            <div className="absolute bottom-8 left-8 right-8 p-6 rounded-2xl bg-neutral-950/60 backdrop-blur-xl border border-white/10">
              ...
              <h3 className="text-xl font-bold text-white mb-2">{t(`${serviceKey}.title`)}</h3>
              <p className="text-sm text-neutral-400 line-clamp-2">{t(`${serviceKey}.focus`)}</p>
            </div>
        ...
        <div className="md:col-span-7 space-y-12">
          ...
          <div className="grid grid-cols-1 gap-6">
            <div className="p-8 bg-white/[0.02] border border-white/[0.05] rounded-[2.5rem] relative overflow-hidden group/box">
              ...
              <h4 className="text-[10px] font-black text-red-500 uppercase tracking-widest mb-3">{t("problem_label")}</h4>
              <p className="text-xl text-neutral-300 font-medium leading-relaxed">{t(`${serviceKey}.problem`)}</p>
            </div>
            <div className="p-8 bg-emerald-500/[0.03] border border-emerald-500/10 rounded-[2.5rem] relative overflow-hidden group/box shadow-[0_20px_40px_-20px_rgba(16,185,129,0.1)]">
              ...
              <h4 className="text-[10px] font-black text-emerald-500 uppercase tracking-widest mb-3">{t("solution_label")}</h4>
              <p className="text-xl text-white font-medium leading-relaxed">{t(`${serviceKey}.solution`)}</p>
            </div>
          </div>
            ...
            <div className="flex flex-col gap-8">
              <div className="flex flex-col gap-2">
                <span className="text-3xl font-black text-white tracking-tight">
                  {isCustomSystem ? t("pricing_breakdown.ai.investment_notes.1").split(": ")[1] : 
                   isSEO ? t("pricing_breakdown.seo.investment.0.value") : 
                   t(`${serviceKey}.pricing_label`)}
                </span>
                {(isCustomSystem || isSEO) && (
                  <span className="text-sm text-neutral-500 italic">
                    {isCustomSystem ? `(${t("pricing_breakdown.ai.investment_notes.2")})` : `(${t("pricing_breakdown.seo.note")})`}
                  </span>
                )}
              </div>
              ...
              <div className="flex flex-wrap items-center gap-6">
                ...
                >
                  {isCustomSystem ? <Calendar className="w-5 h-5" /> : isSEO ? <Search className="w-5 h-5" /> : <ArrowRight className="w-5 h-5" />}
                  {isCustomSystem ? t("cta_labels.scoping_call") : isSEO ? t("cta_labels.site_audit") : t("cta_labels.default")}
                ...
                >
                  {showPricing ? t("pricing_breakdown.labels.hide") : t("pricing_breakdown.labels.show")}
                  <ChevronDown className={cn("w-4 h-4 transition-transform duration-300", showPricing && "rotate-180")} />
                </button>
              </div>
              ...
              {isCustomSystem && (
                <div className="p-8 rounded-[2rem] bg-neutral-900/50 border-l-4 border-emerald-500 space-y-6">
                  ...
                  <ul className="grid grid-cols-1 sm:grid-cols-2 gap-x-12 gap-y-3">
                    {t.raw("scoping_call_box.items").map((point: string, i: number) => (
                  ...
                  <p className="text-[10px] text-neutral-600 font-black uppercase tracking-[0.3em] pt-2">{t("scoping_call_box.footer")}</p>
                </div>
              )}
          ...
          {/* Best For section */}
          <div className="pt-8 border-t border-neutral-800">
            <h4 className="text-[10px] font-black text-neutral-500 uppercase tracking-widest mb-8">{t("best_for.label")}</h4>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              {industries.map((item: any, i: number) => {
                const Icon = ICON_MAP[item.icon] || Check;
                return (
                  <div key={i} className="flex items-center gap-4 p-4 rounded-xl border border-neutral-800/50 bg-white/[0.01] hover:border-emerald-500/30 transition-all group/ind">
                    ...
                    <span className="text-sm font-bold text-neutral-300 group-hover/ind:text-white transition-colors">{item.text}</span>
                  </div>
                );
              })}
            </div>
                ...
                >
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                    {examples.map((example: any, i: number) => (
                      <div key={i} className="p-6 rounded-2xl bg-neutral-900/50 border border-neutral-800/50">
                        <p className="text-xs font-black text-emerald-500 mb-2 uppercase tracking-widest">{example.name}</p>
                        <p className="text-sm text-neutral-400 leading-relaxed font-medium">{example.desc}</p>
                      </div>
                    ))}
                  </div>
                  <Link 
                  ...
                  >
                    {t("real_examples.view_more")} <ExternalLink className="w-3 h-3" />
                  </Link>
                </motion.div>
              )}
            </AnimatePresence>
...
export default function ServicesClient() {
  ...
  return (
    <main className="min-h-screen bg-transparent pt-48 pb-0 relative z-0 selection:bg-emerald-500/30">
      ...
      <section id="experience" className="mb-32 min-h-[800px] scroll-mt-48 w-full max-w-7xl mx-auto px-6 md:px-8">
        ...
        <div className="max-w-7xl mx-auto px-6 md:px-8">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-12 md:gap-8 text-center">
            <div className="space-y-4 group">
              ...
              <span className="text-[10px] font-black text-emerald-500 uppercase tracking-[0.3em]">STANDARD 01</span>
              <div className="text-3xl md:text-5xl font-black tracking-tight text-white group-hover:text-emerald-400 transition-colors">{t("metrics.lighthouse")}</div>
            </div>
            <div className="space-y-4 group">
              <StandardIcons.Worldwide />
              <span className="text-[10px] font-black text-emerald-500 uppercase tracking-[0.3em]">STANDARD 02</span>
              <div className="text-3xl md:text-5xl font-black tracking-tight text-white group-hover:text-emerald-400 transition-colors">{t("metrics.latency")}</div>
            </div>
            <div className="space-y-4 group">
              <StandardIcons.Secure />
              <span className="text-[10px] font-black text-emerald-500 uppercase tracking-[0.3em]">STANDARD 03</span>
              <div className="text-3xl md:text-5xl font-black tracking-tight text-white group-hover:text-emerald-400 transition-colors">{t("metrics.uptime")}</div>
            </div>
          </div>
        </div>
      </div>
      ...
      <div id="comparison" className="scroll-mt-32 max-w-7xl mx-auto px-6 md:px-8">
        <ComparisonTable />
      </div>
      ...
      <div id="contact" className="scroll-mt-32 max-w-7xl mx-auto px-6 md:px-8 pb-32">
      ...
      <div className="lg:hidden fixed bottom-0 left-0 w-full p-4 z-50 pointer-events-none">
        ...
        >
          <div className="flex flex-col">
            <span className="text-[8px] font-black text-emerald-500 uppercase tracking-widest">{activeTab.toUpperCase()}</span>
            <span className="text-xs font-bold text-white">
              {activeTab === "ai" ? t("pricing_breakdown.ai.investment_notes.1").split(": ")[1] : 
               activeTab === "seo" ? t("pricing_breakdown.seo.investment.0.value") : 
